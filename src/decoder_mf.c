 ...
 * Foundation, Inc., 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA
  ...
  Original patch: +++ b/libavcodec/mf.c @@ -0,0 +1,2160 @@
  Author: Pawel Wegner
...
// type definitions for private method signatures from AVCodec

typedef HRESULT (WINAPI *MFGetAttributeSizeProc)(IMFAttributes *pattr, REFGUID guid, UINT32 *pw, UINT32 *ph);
typedef HRESULT (WINAPI *MFSetAttributeSizeProc)(IMFAttributes *pattr, REFGUID guid, UINT32 uw, UINT32 uh);
typedef char * (WINAPI *hr_str_bufProc)(char *buf, size_t size, HRESULT hr);
typedef IMFSample * (WINAPI *create_memory_sampleProc)(void *fill_data, size_t size, size_t align);
typedef enum AVSampleFormat (WINAPI *media_type_to_sample_fmtProc)(IMFAttributes *type);
typedef enum AVPixelFormat (WINAPI *media_type_to_pix_fmtProc)(IMFAttributes *type);
typedef int (WINAPI *fourcc_from_guidProc)(const GUID *guid, uint32_t *out_fourcc);
typedef void (WINAPI *media_type_dumpProc)(void *log, IMFMediaType *type);
typedef const CLSID *(WINAPI *codec_to_mf_subtypeProc)(enum AVCodecID codec);
typedef int (WINAPI *instantiate_mfProc)(void *log, GUID category, MFT_REGISTER_TYPE_INFO *in_type, MFT_REGISTER_TYPE_INFO *out_type, int use_hw, IMFTransform **res);
typedef void (WINAPI *free_mfProc)(IMFTransform **mft);
typedef int (WINAPI *decode_get_packetProc)(AVCodecContext *avctx, AVPacket *pkt);
typedef int (WINAPI *decode_frame_propsProc)(AVCodecContext *avctx, AVFrame *frame);
typedef int (WINAPI *get_bufferProc)(AVCodecContext *avctx, AVFrame *frame, int flags);
typedef int (WINAPI *set_dimensionsProc)(AVCodecContext *s, int width, int height);
typedef int (WINAPI *get_formatProc)(AVCodecContext *avctx, const enum AVPixelFormat *fmt);
typedef int (WINAPI *avpriv_mpeg4audio_get_config2Proc)(MPEG4AudioConfig *c, const uint8_t *buf, int size, int sync_extension, void *logctx);

typedef struct AVPrivateMethods
{
    MFGetAttributeSizeProc          pfn_MFGetAttributeSize;
    MFSetAttributeSizeProc          pfn_MFSetAttributeSize;
    hr_str_bufProc                  pfn_hr_str_buf;
    create_memory_sampleProc        pfn_create_memory_sample;
    media_type_to_sample_fmtProc    pfn_media_type_to_sample_fmt;
    media_type_to_pix_fmtProc       pfn_media_type_to_pix_fmt;
    fourcc_from_guidProc            pfn_fourcc_from_guid;
    media_type_dumpProc             pfn_media_type_dump;
    codec_to_mf_subtypeProc         pfn_codec_to_mf_subtype;
    instantiate_mfProc              pfn_instantiate_mf;
    free_mfProc                     pfn_free_mf;
    decode_get_packetProc           pfn_decode_get_packet;
    decode_frame_propsProc          pfn_decode_frame_props;
    get_bufferProc                  pfn_get_buffer;
    set_dimensionsProc              pfn_set_dimensions;
    get_formatProc                  pfn_get_format;
    avpriv_mpeg4audio_get_config2Proc   pfn_avpriv_mpeg4audio_get_config2;
} AVPrivateMethods;

static AVPrivateMethods shims = {
    .pfn_MFGetAttributeSize = NULL,
    .pfn_MFSetAttributeSize = NULL,
    .pfn_hr_str_buf = NULL,
    .pfn_create_memory_sample = NULL,
    .pfn_media_type_to_sample_fmt = NULL,
    .pfn_media_type_to_pix_fmt = NULL,
    .pfn_fourcc_from_guid = NULL,
    .pfn_media_type_dump = NULL, 
    .pfn_codec_to_mf_subtype = NULL,
    .pfn_instantiate_mf = NULL,
    .pfn_free_mf = NULL,
    .pfn_decode_get_packet = NULL,
    .pfn_decode_frame_props = NULL,
    .pfn_get_buffer = NULL,
    .pfn_set_dimensions = NULL,
    .pfn_get_format = NULL,
    .pfn_avpriv_mpeg4audio_get_config2 = NULL,
};

// shim bindings
#pragma region "Shim Wrappers"
HRESULT ff_MFGetAttributeSize(IMFAttributes *pattr, REFGUID guid, UINT32 *pw, UINT32 *ph)
{
    return shims.pfn_MFGetAttributeSize(pattr, guid, pw, ph);
}
HRESULT ff_MFSetAttributeSize(IMFAttributes *pattr, REFGUID guid, UINT32 uw, UINT32 uh)
{
    return shims.pfn_MFSetAttributeSize(pattr, guid, uw, uh);
}
char *ff_hr_str_buf(char *buf, size_t size, HRESULT hr)
{
    return shims.pfn_hr_str_buf(buf, size, hr);
}
IMFSample *ff_create_memory_sample(void *fill_data, size_t size, size_t align)
{
    return shims.pfn_create_memory_sample(fill_data, size, align);
}
enum AVSampleFormat ff_media_type_to_sample_fmt(IMFAttributes *type)
{
    return shims.pfn_media_type_to_sample_fmt(type);
}
enum AVPixelFormat ff_media_type_to_pix_fmt(IMFAttributes *type)
{
    return shims.pfn_media_type_to_pix_fmt(type);
}
int ff_fourcc_from_guid(const GUID *guid, uint32_t *out_fourcc)
{
    return shims.pfn_fourcc_from_guid(guid, out_fourcc);
}
void ff_media_type_dump(void *log, IMFMediaType *type)
{
    shims.pfn_media_type_dump(log, type);
}
const CLSID *ff_codec_to_mf_subtype(enum AVCodecID codec)
{
    return shims.pfn_codec_to_mf_subtype(codec);
}
...
{
    return shims.pfn_instantiate_mf(log, category, in_type, out_type, use_hw, res);
}
void ff_free_mf(IMFTransform **mft)
{
    shims.pfn_free_mf(mft);
}
int ff_decode_get_packet(AVCodecContext *avctx, AVPacket *pkt)
{
    return shims.pfn_decode_get_packet(avctx, pkt);
}
int ff_decode_frame_props(AVCodecContext *avctx, AVFrame *frame)
{
    return shims.pfn_decode_frame_props(avctx, frame);
}
int ff_get_buffer(AVCodecContext *avctx, AVFrame *frame, int flags)
{
    return shims.pfn_get_buffer(avctx, frame, flags);
}
int ff_set_dimensions(AVCodecContext *s, int width, int height)
{
    return shims.pfn_set_dimensions(s, width, height);
}
int ff_get_format(AVCodecContext *avctx, const enum AVPixelFormat *fmt)
{
    return shims.pfn_get_format(avctx, fmt);
}
int avpriv_mpeg4audio_get_config2(MPEG4AudioConfig *c, const uint8_t *buf, int size, int sync_extension, void *logctx)
{
    return shims.pfn_avpriv_mpeg4audio_get_config2(c, buf, size, sync_extension, logctx);
}
#pragma endregion

// dynamic private method import and shim wireup
static void ff_bind_av_private_methods()
{
    HANDLE dll = GetModuleHandleW(L"avcodec.dll");
    if (dll) {
        shims.pfn_MFGetAttributeSize = (MFGetAttributeSizeProc)GetProcAddress(dll, "ff_MFGetAttributeSize");
        shims.pfn_MFSetAttributeSize = (MFSetAttributeSizeProc)GetProcAddress(dll, "ff_MFSetAttributeSize");
        shims.pfn_hr_str_buf = (hr_str_bufProc)GetProcAddress(dll, "ff_hr_str_buf");
        shims.pfn_create_memory_sample = (create_memory_sampleProc)GetProcAddress(dll, "ff_create_memory_sample");
        shims.pfn_media_type_to_sample_fmt = (media_type_to_sample_fmtProc)GetProcAddress(dll, "ff_media_type_to_sample_fmt");
        shims.pfn_media_type_to_pix_fmt = (media_type_to_pix_fmtProc)GetProcAddress(dll, "ff_media_type_to_pix_fmt");
        shims.pfn_fourcc_from_guid = (fourcc_from_guidProc)GetProcAddress(dll, "ff_fourcc_from_guid");

        shims.pfn_media_type_dump = (media_type_dumpProc)GetProcAddress(dll, "ff_media_type_dump"); 
        shims.pfn_codec_to_mf_subtype = (codec_to_mf_subtypeProc)GetProcAddress(dll, "ff_codec_to_mf_subtype"); 
        shims.pfn_instantiate_mf = (instantiate_mfProc)GetProcAddress(dll, "ff_instantiate_mf"); 
        shims.pfn_free_mf = (free_mfProc)GetProcAddress(dll, "ff_free_mf"); 
        shims.pfn_decode_get_packet = (decode_get_packetProc)GetProcAddress(dll, "ff_decode_get_packet"); 
        shims.pfn_decode_frame_props = (decode_frame_propsProc)GetProcAddress(dll, "ff_decode_frame_props"); 
        shims.pfn_get_buffer = (get_bufferProc)GetProcAddress(dll, "ff_get_buffer"); 
        shims.pfn_set_dimensions = (set_dimensionsProc)GetProcAddress(dll, "ff_set_dimensions"); 
        shims.pfn_get_format = (get_formatProc)GetProcAddress(dll, "ff_get_format"); 
        shims.pfn_avpriv_mpeg4audio_get_config2 = (avpriv_mpeg4audio_get_config2Proc)GetProcAddress(dll, "avpriv_mpeg4audio_get_config2"); 
        FreeModule(dll);
    }
}
...
{
    const CLSID *subtype = ff_codec_to_mf_subtype(id);
...
#if CONFIG_SMALL
#   define ABSENT_IF_CONFIG_SMALL(PROP,VAL)
#else
#   define ABSENT_IF_CONFIG_SMALL(PROP,VAL) . ## PROP ## = VAL, 
#endif

#define OFFSET(x) offsetof(MFDecoderContext, x)
...
#define MF_DECODER(MEDIATYPE, NAME, ID, OPTS) \
    ...
    AVCodec ff_ ## NAME ## _mf_decoder = {                                     \
        ...
        .name           = #NAME "_mf",                                         \
        ABSENT_IF_CONFIG_SMALL(long_name, #ID " via MediaFoundation")          \
